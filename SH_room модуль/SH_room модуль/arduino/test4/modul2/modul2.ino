...
#define AUTO_OFF_DISPLAY_TIME        30       // авто погашение экрана время (в секундах)
...
void loop() {
  ...
  myButt1.tick(analog < 400 && analog > 200);
  myButt3.tick(analog < 700 && analog > 500);
  myButt2.tick(analog < 1030 && analog > 800);
  ...
  if (millis() - Timer_auto_off_display > AUTO_OFF_DISPLAY_TIME * 1000 and IS_AUTO_OFF_DISPLAY) {
    is_on_scrine = 0;
    oled.setPower(is_on_scrine);
  }

  // таймер авто обновление дисплея
  if (millis() - Timer_auto_draw > AUTO_DRAW_DISPLAY * 1000 and is_on_scrine) {
...
void draw_display() {
  ...
  if (!initil) {
    ...
    Timer_auto_off_display = millis();
    oled.home();
    oled.setCursor(0, 1); oled.print("    Device is not");
    oled.setCursor(0, 2); oled.print("     Initialized!");
    oled.setCursor(0, 4); oled.print("Check the connection");
    oled.setCursor(0, 5); oled.print("to the main module");
  } else {
    ...
    if (is_client) {
      ...
      switch (_str_num) {
        ...
        case 3:
          ...
          oled.setCursor(0, 4); if (muted) oled.print("Event: " + String(warning) + " - !MUTED!"); else oled.print("Event: " + String(warning) + "            ");
...
void compil() {
  String mes = "";
  if (IS_MOVE) {
    mes = "CiSHP-1m|t1|" + String(error_tupe) + "|" + String(warning) + "|" + String(temp) + "|" + String(higr) + "|" + String(is_client) + "|" + String(hour_start) + ":" + String(min_start) + ":" + String(sec_start) + "|" + String(rele1.is_on) + "," + String(rele2.is_on) + "," + String(rele3.is_on) + "," + String(rele4.is_on) + "," + String(rele5.is_on) + "," + String(rele6.is_on) + "," + String(rele7.is_on) + "," + String(rele8.is_on) + "|" +String(time_move);
    Serial.println(mes);
  }
  if (!IS_MOVE) {
    mes = "CiSHP-1m|t2|<error>" + String(error_tupe) + "<error>" +
          "<warn>" + String(warning) + "<warn>" +
          "<temp>" + String(temp) + "<temp>" +
          "<hum>" + String(higr) + "<hum>" +
          "<client>" + String(is_client) + "<client>" +
          "<time_start>" + String(hour_start) + ":" + String(min_start) + ":" + String(sec_start) + "<time_start>" +
          "<relay>" + String(rele1.is_on) + "," + String(rele2.is_on) + "," + String(rele3.is_on) + "," + String(rele4.is_on) + "," + String(rele5.is_on) + "," + String(rele6.is_on) + "," + String(rele7.is_on) + "," + String(rele8.is_on) + "<relay>" +
          "<time_move>n<time_move>";
  }
